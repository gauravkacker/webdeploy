...
export default function LicensingPage() {
    ...
    try {
      ...
      if (licensesRes.ok) {
        try {
          ...
          console.warn('Failed to parse licenses response:', e);
        }
      } else {
        console.warn('Failed to fetch licenses:', licensesRes.status);
      }
      ...
      if (plansRes.ok) {
        try {
          ...
          console.warn('Failed to parse plans response:', e);
        }
      }
    } catch (error) {
      console.warn('Failed to load data (non-critical):', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreatePlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`handleCreatePlan: editingPlanId=${editingPlanId}, method=${method}, url=${url}`);
      ...
      console.log(`handleCreatePlan: payload=`, planPayload);
      ...
      console.log(`handleCreatePlan: response status=${res.status}`);
      ...
      if (res.ok) {
        ...
        // Immediately update UI state with the returned data
        if (editingPlanId) {
          // Update existing plan
          setPlans(prevPlans => prevPlans.map(p => p.id === editingPlanId ? data : p));
        } else {
          // Add new plan
        ...
        console.error(`handleCreatePlan: error response=`, errorData);
  ...
  const handleDeletePlan = async (planId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json();
        // If plan not found in database, it's already removed from UI, so just show success
        if (res.status === 404) {
          alert('Plan deleted successfully!');
        } else {
  ...
  const handleEditPlan = (plan: Plan) => {
    ...
    setPlanForm({ 
      ...
      modules: plan.modules && plan.modules.length > 0 ? plan.modules : [],
  ...
  const formatLicenseExpiryDate = (expiresAt: string | Date | undefined): string => {
  ...
  const calculateLicenseDaysRemaining = (expiresAt: string | Date | undefined): number => {
                                    ...
                                    {showPassword === license.id ? (
                                      <div className="space-y-1">
                                        ...
                                        <div className="text-xs text-green-700 space-y-0.5">
                                          <p>License Expiry: {formatLicenseExpiryDate(license.expiresAt)}</p>
                                          <p>Days Remaining: <span className={calculateLicenseDaysRemaining(license.expiresAt) <= 3 ? 'text-red-600 font-semibold' : ''}>{calculateLicenseDaysRemaining(license.expiresAt)}</span> days</p>
                                        </div>
                                        <Button
                            ...
                            {showPassword === selectedLicense.id ? (
                              <div className="space-y-2">
                                ...
                                <div className="text-xs text-green-700 space-y-1">
                                  <p>License Expiry: {formatLicenseExpiryDate(selectedLicense.expiresAt)}</p>
                                  <p>Days Remaining: <span className={calculateLicenseDaysRemaining(selectedLicense.expiresAt) <= 3 ? 'text-red-600 font-semibold' : ''}>{calculateLicenseDaysRemaining(selectedLicense.expiresAt)}</span> days</p>
                                </div>
                                <Button
