...
import { appointmentDb, patientDb, billingReceiptDb, billingQueueDb, medicineBillDb, feeHistoryDb } from "@/lib/db/database";
import { doctorPrescriptionDb, doctorVisitDb, pharmacyQueueDb } from "@/lib/db/doctor-panel";
 ...
 export default function ReportsPage() {
   ...
   // Check authentication on mount and set dates
   useEffect(() => {
     setIsClient(true);
     ...
     const date = new Date();
     date.setDate(1);
     const monthStartDate = date.toISOString().split("T")[0];
  ...
  const [phStart, setPhStart] = useState<string>("");
  const [phEnd, setPhEnd] = useState<string>("");
  const [feeType, setFeeType] = useState<"daily" | "range" | "outstanding" | "refunds" | "bills">("range");
  const [feeDate, setFeeDate] = useState<string>("");
  const [feeStart, setFeeStart] = useState<string>("");
  const [feeEnd, setFeeEnd] = useState<string>("");
  const [feePayMethod, setFeePayMethod] = useState<"all" | "cash" | "card" | "upi" | "cheque" | "insurance" | "exempt">("all");
  const [aptStart, setAptStart] = useState<string>("");
  const [aptEnd, setAptEnd] = useState<string>("");
  const [ptStart, setPtStart] = useState<string>("");
  const [ptEnd, setPtEnd] = useState<string>("");
  const [rxStart, setRxStart] = useState<string>("");
  const [rxEnd, setRxEnd] = useState<string>("");
  const [fuStart, setFuStart] = useState<string>("");
  const [fuEnd, setFuEnd] = useState<string>("");
  const [upStart, setUpStart] = useState<string>("");
  const [upEnd, setUpEnd] = useState<string>("");
  const [billStatus, setBillStatus] = useState<"all" | "paid" | "pending" | "partial">("all");
  
  // Initialize date states when today and monthStart are set
  useEffect(() => {
    if (today && monthStart) {
      setPhStart(monthStart);
      setPhEnd(today);
      setFeeDate(today);
      setFeeStart(monthStart);
      setFeeEnd(today);
      setAptStart(monthStart);
      setAptEnd(today);
      setPtStart(monthStart);
      setPtEnd(today);
      setRxStart(monthStart);
      setRxEnd(today);
      setFuStart(monthStart);
      setFuEnd(today);
      setUpStart(monthStart);
      setUpEnd(today);
    }
  }, [today, monthStart]);
  ...
  const data = useMemo((): { headers: string[]; rows: Row[] } => {
    ...
    if (activeTab === "fees") {
      if (feeType === "daily") {
        let fees = (feeHistoryDb.getAll() as Array<any>).filter((f) => {
          const d = new Date(f.paidDate).toISOString().split("T")[0];
          // Filter out fees for deleted patients
          const patient = patientDb.getById(f.patientId);
          return d === feeDate && patient !== undefined && patient !== null;
        });
        if (feePayMethod !== "all") {
          fees = fees.filter((f) => (f.paymentMethod || "").toLowerCase() === feePayMethod);
        }
        ...
        fees.forEach((f) => {
          if (f.visitId) {
            const existing = visitMap.get(f.visitId);
            if (!existing || new Date(f.paidDate) > new Date(existing.paidDate)) {
              visitMap.set(f.visitId, f);
            }
          } else {
        ...
        fees = Array.from(visitMap.values());
        
        // Calculate total from filtered fees only
        const total = fees.reduce((sum, f) => sum + (typeof f.amount === "number" ? f.amount : 0), 0);
        
        const rows: Row[] = fees.map((f) => {
          const p = patientDb.getById(f.patientId) as any;
          const name = p ? `${p.firstName} ${p.lastName}` : "Unknown";
          const amount = typeof f.amount === "number" ? f.amount.toFixed(2) : `${f.amount}`;
          return { id: f.id, cols: [formatDate(f.paidDate), f.id, name, amount, (f.paymentMethod || "").toUpperCase()], patientId: f.patientId, receiptNumber: f.id };
        });
             ...
             {data.rows.length === 0 && activeTab !== "fees" ? (
               ...
               <div className="overflow-x-auto">
                {activeTab === "fees" && (feeType === "daily" || feeType === "range") && (() => {
                  ...
                  // Read from feeHistoryDb instead of billingReceiptDb
                  let fees = (feeHistoryDb.getAll() as Array<any>).filter((f) => {
                    const d = new Date(f.paidDate);
                    // Filter out fees for deleted patients
                    const patient = patientDb.getById(f.patientId);
                  ...
                  // Calculate totals from feeHistoryDb
                  const billed = fees.reduce((sum, f) => sum + (f.amount || 0), 0);
                  const collected = billed; // All fees in feeHistoryDb are collected (they're created when fee is paid)
                  const refunded = 0; // Refunds would be tracked separately if needed
                  const pending = 0; // Pending fees are not in feeHistoryDb yet
                  
                  const breakdownMap: Record<string, { count: number; amount: number }> = {};
                  fees.forEach((f) => {
                    const desc = f.feeType || "consultation";
                    if (!breakdownMap[desc]) breakdownMap[desc] = { count: 0, amount: 0 };
                    breakdownMap[desc].count += 1;
                    breakdownMap[desc].amount += f.amount || 0;
                  });
