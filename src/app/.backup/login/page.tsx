...
import { userDb, roleDb, ensureModule2DataSeeded, db } from "@/lib/db/database";
import type { User, Role } from "@/types";

const CURRENT_USER_KEY = 'clinic_current_user';
const REMEMBER_ME_KEY = 'clinic_remember_me';
const ADMIN_USERS_KEY = 'clinic_users'; // Admin-created users
...
export default function LoginPage() {
  ...
  // Check if already authenticated - but don't redirect, just let them stay on login
  // The root page will handle redirects properly
  useEffect(() => {
    // Don't do anything - let the user stay on login page
    // If they're already authenticated, they can navigate manually or the app will handle it
  }, []);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If normal password doesn't match, try license password activation
      let licenseActivationData: any = null;
      let licenseActivationError: any = null;
      if (!isPasswordValid) {
        console.log('Normal password failed, trying license password activation...');
        try {
          // Get machine ID for license activation
          const machineIdResponse = await fetch('/api/license/machine-id');
          const machineIdData = await machineIdResponse.json();
          const machineId = machineIdData.machineId;

          if (!machineId) {
            throw new Error('Could not determine machine ID');
          }

          // Try to activate with password
          const activationResponse = await fetch('/api/license/activate-with-password', {
            method: 'POST',
          ...
          if (activationResponse.ok && activationData.success) {
            console.log('License password activation successful');
            isPasswordValid = true;
            licenseActivationData = { ...activationData, machineId };
          } else {
            console.log('License password activation failed:', activationData.error);
            // Store error for display
            licenseActivationError = activationData;
          }
        } catch (licenseError) {
          console.error('License password activation error:', licenseError);
          // Fall through to normal password error
        }
      }

      if (!isPasswordValid) {
        setError(licenseActivationError?.error || 'Invalid username or password');
        setIsLoading(false);
        return;
      }
      ...
      // Set license_key cookie for middleware authentication
      // This allows middleware to recognize the user as authenticated
      document.cookie = `license_key=${user.id}; path=/; max-age=${7 * 24 * 60 * 60}`;
      console.log('Set license_key cookie for middleware');
      
      console.log('Login successful, redirecting...');
      
      // If this was a license password activation, show the activation confirmation page
      if (licenseActivationData) {
        const activationDetails = {
          machineId: licenseActivationData.machineId,
          licenseKey: licenseActivationData.licenseKey,
          plan: licenseActivationData.plan,
          activationDate: new Date().toISOString(),
          expiryDate: licenseActivationData.expiryDate,
          daysRemaining: licenseActivationData.daysUntilExpiry,
          maxMachines: licenseActivationData.maxMachines,
        };
        sessionStorage.setItem('licenseActivationDetails', JSON.stringify(activationDetails));
        
        // Use a small delay to ensure cookie is set before navigation
        setTimeout(() => {
          router.push('/license-activated');
        }, 100);
      } else {
        // Normal login, redirect to queue (main software)
        // Use a small delay to ensure cookie is set before navigation
        setTimeout(() => {
          router.push('/queue');
        }, 100);
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm">
              {error}
              {licenseActivationError?.alreadyRegistered && (
                <div className="mt-3 pt-3 border-t border-red-200">
                  <p className="font-semibold mb-2">Device Already Registered</p>
                  <p className="text-xs mb-2">{licenseActivationError.message}</p>
                  <p className="text-xs">
                    PCs remaining: <strong>{licenseActivationError.pcsRemaining}</strong>
                  </p>
                  <p className="text-xs">
                    Password expires: <strong>{new Date(licenseActivationError.passwordExpiryDate).toLocaleDateString()}</strong>
                  </p>
                </div>
              )}
            </div>
