...
export default function OnlineAppointmentsSettings() {
  ...
  const handleStartWaServer = async () => {
    setWaStarting(true);
    try {
      // Call IPC directly to start the WhatsApp server
      if (typeof window !== 'undefined' && (window as any).electronAPI?.whatsappStart) {
        const result = await (window as any).electronAPI.whatsappStart();
        if (result.success) {
          setWaServerError(false);
          ...
          setWaServerError(true);
          console.error('[WhatsApp] Start failed:', result.error);
        }
      } else {
        // Fallback for web mode (no-op)
        console.log('[WhatsApp] Not in Electron mode, skipping server start');
      }
    } catch (err) {
      setWaServerError(true);
      console.error('[WhatsApp] Error starting server:', err);
    } finally {
      setWaStarting(false);
    }
  };
            ...
            {appointmentSource === "whatsappParser" && (
              <div className={`mb-4 flex items-center justify-between px-4 py-2.5 rounded-lg border text-sm ${
                ...
                <div className="ml-auto flex items-center gap-2">
                  {waStatus === 'connected' ? (
                    <button onClick={handleWaDisconnect} className="text-xs text-gray-500 hover:text-red-600 underline">Disconnect</button>
                  ) : (
                    <button onClick={handleWaDisconnect} className="text-xs text-gray-500 hover:text-blue-600 underline">Reset Connection</button>
                  )}
                  <button 
                  ...
                  >
                    {waStarting ? '⏳ Starting...' : '▶ Start WhatsApp'}
                  </button>
                </div>
              </div>
            )}
