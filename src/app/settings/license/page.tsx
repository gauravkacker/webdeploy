...
export default function LicenseSettingsPage() {
  ...
  const handleDeregister = async () => {
    ...
    try {
      const response = await fetch('/api/license/deregister', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok || !data.success) {
        setDeregisterError(data.error || 'Failed to deregister license');
        setIsDeregisterLoading(false);
        return;
      }
      ...
      setDeregisterError('Failed to connect to server');
      setIsDeregisterLoading(false);
    }
  };
            ...
            <div className="flex items-start justify-between mb-6">
              ...
              {(user?.role === 'admin' || user?.role === 'doctor') && (
                <div className="flex gap-2">
                  ...
                  {licenseInfo?.isValid && (
                    <Button variant="danger" onClick={() => setShowDeregisterModal(true)}>
                      Deregister License
                    </Button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-md p-6">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Deregister License?</h2>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                <p className="text-sm text-yellow-800">
                  This will remove your license binding from this computer. You will be able to register with a new license.
                </p>
              </div>
              ...
              {licenseInfo?.daysRemaining !== undefined && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-sm text-blue-800">
                    Your current license has <span className="font-semibold">{licenseInfo.daysRemaining >= 3650 ? 'Lifetime' : `${licenseInfo.daysRemaining} days`}</span> remaining.
                  </p>
                </div>
              )}
              ...
              {deregisterError && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                  <p className="text-sm text-red-800">{deregisterError}</p>
                </div>
              )}
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={isDeregisterLoading}
                >
                  {isDeregisterLoading ? 'Deregistering...' : 'Deregister'}
                </Button>
              </div>
            </div>
          </Card>
