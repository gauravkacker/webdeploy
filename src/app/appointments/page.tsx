...
export default function AppointmentsPage() {
  ...
  const loadAppointments = useCallback(() => {
    ...
    // Deduplicate appointments by ID (in case of race conditions)
    const seenIds = new Set<string>();
    const deduplicatedFiltered = filtered.filter((apt) => {
      if (seenIds.has(apt.id)) {
        console.warn(`[Appointments] Duplicate appointment ID detected: ${apt.id}, filtering out`);
        return false;
      }
      seenIds.add(apt.id);
      return true;
    ...
    const sortedAppointments = deduplicatedFiltered.sort((a, b) => {
