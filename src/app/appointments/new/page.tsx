...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    if (debouncedSearchQuery.trim()) {
      ...
      setSearchResults(deduplicatePatients(results).filter(p => !bookedPatientIds.has(p.id)).slice(0, 20));
    } else {
      setSearchResults([]);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push("/appointments");
      ...
      alert('Error booking appointment: ' + (error instanceof Error ? error.message : String(error)));
    } finally {
